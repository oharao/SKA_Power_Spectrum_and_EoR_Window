...
"""
def get_Pd_avg(name1,name2,filepath,N_baselines,freq_values,freq_interval,
time_samples,Dc,delta_Dc,wavelength,bandwidth,z,window_beta):
    ...
    #delay_values = np.delete(delay_values,0,0)
    delay_values = np.abs(delay_values) #put here from delay_transform
    sorted_delay_values = np.sort(delay_values)
    sum_sorted_vis = np.zeros([len(sorted_delay_values)//2+1,N_baselines])
    #the // is purely there to make it an int
    ...
    for j in np.arange(time_samples):
        ...
        #vis_data = np.delete(vis_data,0,0) #getting rid of zero value
        ...
        #delay_values comes in doubles
        if len(freq_values)%2 == 1:
            return "Error, number of frequencies values has to be even, if you ever need to analyse an odd number of frequencies, write code here..."
        else:
            reduced_sorted_delay_values = np.zeros([len(sorted_delay_values)//2 +1])
            reduced_sorted_vis_delay = np.zeros([len(sorted_delay_values)//2 +1,N_baselines])
            reduced_sorted_delay_values[0] = sorted_delay_values[0]
            reduced_sorted_delay_values[-1] = sorted_delay_values[-1]
            reduced_sorted_vis_delay[0,:] = sorted_vis_delay[0,:]/2
            """The zero value is already folded over, hence divide by 2"""
            reduced_sorted_vis_delay[-1,:] = sorted_vis_delay[-1,:]

            for i in range(len(sorted_delay_values)//2 -1):
                reduced_sorted_delay_values[i+1]=sorted_delay_values[2*i+1]
                reduced_sorted_vis_delay[i+1,:]=(sorted_vis_delay[2*i+1,:]+sorted_vis_delay[2*i+2,:])/2

        sum_sorted_vis = sum_sorted_vis + reduced_sorted_vis_delay
    ...
    ########## the k axes
    k_parallel = get_k_parallel(z,reduced_sorted_delay_values)
    k_interval = k_parallel[2]-k_parallel[1]
    k_parallel_plot = np.arange(k_parallel[1]-k_interval/2,k_parallel[-1]+k_interval*3/2,
    k_interval)
    #k_parallel_plot = np.insert(k_parallel_plot,0,0) #bin edge values for pcolormesh

    k_perp = get_k_perp(sorted_baseline_mag,freq_values[20],Dc)
    ##################

    return P_d,k_parallel_plot,k_perp

"""
eor plot with visibilities sum up in blocks, returns grid of Pd, k_parallel and
k_perp values for plotting
"""
def get_Pd_blocks(name1,name2,filepath,N_baselines,freq_values,freq_interval,
time_samples,Dc,delta_Dc,wavelength,bandwidth,z,N_blocks,window_beta):
    #every 130816th baselines are of the same magnitude because it is the same pair rotated around Earth
    baseline_mag = get_baselines_mag(name1,name2,filepath,0,N_baselines)
    sorted_baseline_mag = np.sort(baseline_mag)
    vis_position = get_vis_boundaries(sorted_baseline_mag,N_blocks)[0]
    delay_values = get_delay_times(freq_values,freq_interval)
    delay_values = np.delete(delay_values,0,0)
    delay_values = np.abs(delay_values) #put here from delay_transform
    sorted_delay_values = np.sort(delay_values)
    sum_sorted_vis = np.zeros([len(freq_values)//2,N_blocks])

    for j in np.arange(time_samples):
        vis_data = delay_transform(name1,name2,filepath,j*N_baselines,N_baselines,freq_values,window_beta)
        vis_data = np.delete(vis_data,0,0)
        vis_delay = np.abs(vis_data)**2 #get modulus squared
        #try sorting by baseline magnitude
        sortedi_vis_delay = vis_delay[:,baseline_mag.argsort()]
        sorted_vis_delay = sortedi_vis_delay[delay_values.argsort(),:]
        ...
        """
        Pause here add up vis in blocks
        get_vis_boundaries function basically uses left-bisect, which points to
        the position of the leftmost element in a bin, hence the rightmost element
        from its neighbouring bin is position-1
        However if two successive bin edge values are equal, there aren't any elements
        in that bin, so it has to be 0. (don't need else because array initialised
        as zeros)
        """
        sorted_vis_delay_blocks = np.zeros([39,N_blocks])
        for q in range(N_blocks):
            if vis_position[q] != vis_position[q+1]:
                sorted_vis_delay_blocks[:,q] = np.sum(sorted_vis_delay[:,vis_position[q]:vis_position[q+1]-1], axis=-1)

        reduced_sorted_delay_values = np.zeros([len(sorted_delay_values)//2 +1])
        reduced_sorted_vis_delay = np.zeros([len(sorted_delay_values)//2 +1,N_blocks])
        reduced_sorted_delay_values[-1] = sorted_delay_values[-1]
        reduced_sorted_vis_delay[-1,:] = sorted_vis_delay_blocks[-1,:]


        for i in range(len(sorted_delay_values)//2):
            reduced_sorted_delay_values[i]=sorted_delay_values[2*i]
            reduced_sorted_vis_delay[i,:]=(sorted_vis_delay_blocks[2*i,:]+sorted_vis_delay_blocks[2*i+1,:])/2
    ...
    k_B = 1.38e-23 #Boltzman constant
    #delayed power spectrum, also A_e?? 40*N of gleam
    P_d = avg_sorted_vis*np.divide((Dc**2)*delta_Dc*wavelength**2,(2*k_B*bandwidth)**2)

    ########## the k axes
    k_parallel = get_k_parallel(z,sorted_delay_values)
    k_interval = k_parallel[2]-k_parallel[1]
    k_parallel_plot = np.arange(k_parallel[0]-k_interval/2,k_parallel[-1]+k_interval*3/2,
    k_interval) #bin edge values for pcolormesh

    baseline_block_boundaries = get_vis_boundaries(sorted_baseline_mag,N_blocks)[1]
    k_perp_plot = get_k_perp(baseline_block_boundaries,freq_values[20],Dc)
    ##################

    return P_d,k_parallel_plot,k_perp_plot

"""TESTING FOR UnFOLDED VISIBILTIIES
"""
def get_Pd_unfolded(name1,name2,filepath,N_baselines,freq_values,freq_interval,
time_samples,Dc,delta_Dc,wavelength,bandwidth,z,N_blocks,window_beta):
    #every 130816th baselines are of the same magnitude because it is the same pair rotated around Earth
    baseline_mag = get_baselines_mag(name1,name2,filepath,0,N_baselines)
    sorted_baseline_mag = np.sort(baseline_mag)
    ...
    #delay_values = np.abs(delay_values) #put here from delay_transform
    sorted_delay_values = np.sort(delay_values)
    sum_sorted_vis = np.zeros([len(freq_values)-1,N_blocks]) #-1 for the zero value
    ...
    for j in np.arange(time_samples):
        ...
        sum_sorted_vis = sum_sorted_vis + sorted_vis_delay_blocks
    ...
    k_interval) #bin edge values for pcolormesh #becareful of end values
    k_parallel_plot = np.delete(k_parallel_plot,19)
...
#get_Pd_avg but unfolded (ie no blocks for k_perp) uses linear plot
def get_Pd_avg_unfolded(name1,name2,filepath,N_baselines,freq_values,freq_interval,
time_samples,Dc,delta_Dc,wavelength,bandwidth,z,window_beta):
    #every 130816th baselines are of the same magnitude because it is the same pair rotated around Earth
    baseline_mag = get_baselines_mag(name1,name2,filepath,0,N_baselines)
    sorted_baseline_mag = np.sort(baseline_mag)
    ...
    sum_sorted_vis = np.zeros([len(sorted_delay_values),N_baselines])
    ...
    for j in np.arange(time_samples):
        ...
        sum_sorted_vis = sum_sorted_vis + sorted_vis_delay
...
time_samples,Dc,delta_Dc,wavelength,bandwidth,z,window_beta,N_bins):
    ...
    for j in np.arange(time_samples):
        ...
        for q in range(N_bins):
            if vis_position[q] != vis_position[q+1]:
                sorted_vis_delay_bins[:,q] = np.sum(sorted_vis_delay[:,vis_position[q]:vis_position[q+1]-1], axis=-1)/(vis_position[q+1]-vis_position[q])
...
def main():
    ...
    filepath1 = "./OSKAR_data/test_eor2/"
    filepath2 = "./OSKAR_data/MS_EDGES/"
    ...
    #"CMB_0.1_16.5_10_5000_Freq_"
    #"CMB_0.4_4.2_0.1_500_Freq_"
    #"CMB_0.04_4.2_10_25000_Freq_"


    Dc_file = './OSKAR_data/21cmBiBox/los_comoving_distance.csv'
    Dc_values = get_Dc_values(Dc_file)

    delta_Dc_file = './OSKAR_data/21cmBiBox/delta_los_comoving_distance.csv'
    ...
    freq_interval,time_samples,Dc_values[20],delta_Dc_values[20],wavelength_values[20],
    ...
    c = ax.pcolormesh(k_perp_plot, k_parallel_plot[21:], P_d_gleam[21:,:],norm=LogNorm(vmin=masked_P_d_gleam.min(),vmax=P_d_gleam.max()), cmap="jet")
    ...
    c = ax2.pcolormesh(k_perp_plot, k_parallel_plot, P_d_gleam,norm=LogNorm(vmin=masked_P_d_gleam.min(),vmax=P_d_gleam.max()), cmap="jet")
    ...
    c = ax3.pcolormesh(k_perp_plot, k_parallel_plot[21:], Pd_signal[21:,],norm=LogNorm(vmin=masked_Pd_signal.min(),vmax=Pd_signal.max()), cmap="jet")
    ...
    masked_Pd_signal = np.ma.masked_equal(Pd_signal,0.0,copy=False)
    c = ax4.pcolormesh(k_perp_plot, k_parallel_plot, Pd_signal,norm=LogNorm(vmin=masked_Pd_signal.min(),vmax=Pd_signal.max()), cmap="jet")
    ...
    c = ax5.pcolormesh(k_perp_plot, k_parallel_plot[21:], Pd_relative[21:,:],norm=LogNorm(vmin=masked_Pd_relative.min(),vmax=Pd_relative.max()), cmap="jet")
    ...
    masked_Pd_relative = np.ma.masked_equal(Pd_relative,0.0,copy=False)
    c = ax6.pcolormesh(k_perp_plot, k_parallel_plot, Pd_relative,norm=LogNorm(vmin=masked_Pd_relative.min(),vmax=Pd_relative.max()), cmap="jet")

...
class cable_decay:
    ...
    def __init__(self, max_freq, min_freq, channels, a, b, c, rho_in, rho_out, mu_in, mu_out, roughness, eps_dielectric,
        ...
        self.frequencies = np.linspace(self.min_freq, self.max_freq, channels)  # Hz.
...
def get_antenna_pos():
    ...
    for i, x, y in zip(range(512), station_pos['x'], station_pos['y']):
        df = pd.read_csv('./antenna_pos/station' + str(i).rjust(3, '0') + '/layout.txt', header=None,
...
def compute_interferometer_s21(max_freq=0.108975, min_freq=0.072, channels=1480,
    ...
    ska = cable_decay(max_freq=max_freq, min_freq=min_freq, channels=channels, a=0.0004572, b=0.0014732, c=0.0017272,
                      rho_in=1.67 * 10 ** -8, rho_out=1.67 * 10 ** -8, mu_in=1.0, mu_out=1.0, roughness=0.0,
                      eps_dielectric=2.2,
                      rho_dielectric=1 * 10 ** 18, mu_dielectric=1, tan_d=3 * 10 ** -4, tcr_in=0.00404, tcr_out=0.00404,
                      z_l=60.0)
